...
def generate_perversion():
    K = 1
    tau_0 = 0
    Gamma = (input('Value of Gamma (positive float) - press enter for default value 2/3 - :'))
    if Gamma == '':
        Gamma = 2/3
    else:
        Gamma = float(Gamma)
    epsilon = 1e-4
    tres =0.02
    Lambda = (input('Value of Lambda (positive float) - press enter for default value 1 - :'))
    tailletheta =100
    if Lambda == '':
        Lambda = 1
    else:
        Lambda = float(Lambda)
    axialload = input('Value of axial load (float between 0 and 1/Gamma) - press enter for default value 1/Gamma/2- :')
    if axialload == '':
        axialload = 1/Gamma/2
    else:
        axialload = float(axialload)
    numberofcoils = input('Number of coils of the rod in its natural state (positive float) - press enter for default value 10-:')
    if numberofcoils == '':
        numberofcoils = 10
    else:
    ...
    def three_point_test():
        ...
        for j in range(lenhomo):
            ...
            if np.sqrt(sol_at_this_phase[approx_perv_ind, 3]**2+sol_at_this_phase[approx_perv_ind, 4]**2) < 1:
